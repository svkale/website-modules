...
function putins_make_subpage_from_HTML(dom,doc_ele,element)
{
	...
	if(dom.documentElement.innerText.includes("{StarRating}") && dom.documentElement.innerText.includes("{/StarRating}"))
	{
		...
		let notice_target=dom.evaluate("//*[contains(.,'"+html_text+"')]",dom,null, XPathResult.ANY_TYPE,null);
		let eval_text='gsrfn("fn=StarRating&page="+(document.URL).replace("#","///"),"SRV_mytc");';
		let fs=document.createElement("script");
		fs.setAttribute("type","text/javascript");
		fs.innerText=eval_text;
		document.getElementsByTagName('html')[0].appendChild(fs);
		dom.documentElement.innerHTML=dom.documentElement.innerHTML.replace(doc_html.substring(doc_html.search("{StarRating}"),doc_html.search("{/StarRating}")+12),"");
	}
